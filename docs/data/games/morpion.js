...
Game.prototype={
	...
	install:function(){
		...
		for(i=0;i<this.tab.length;i++){
			for(j=0;j<this.tab[0].length;j++){
				...
				shtml+='<a href="#" onclick="game.add('+i+','+j+');return false;" ><img src="img/morpion-'+ifond+'.png" id="case_'+i+'_'+j+'" /></a>';
	...
	showCase:function(i,j,ifond){
		...
		if(ifond==0){
			getById('case_'+i+'_'+j).src='img/morpion-0.png';
		}else{
			getById('case_'+i+'_'+j).src='img/morpion-'+ifond+'.png';
		}
		/*
